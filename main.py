...
async def adm_p_dec(c: types.CallbackQuery):
    ...
    if act == "v":
        ...
        await bot.send_message(uid, "❌ Toʻlov rad etildi! Chek xato yoki soxta.")

# --- GAMES ---
@dp.message(F.text.in_(["🎮 PUBG Mobile", "🔥 Free Fire", "❄️ Genshin Impact", "⚔️ Mobile Legends"]))
async def g_shop(m, s):
    await s.clear(); t = m.text; p_dict = PUBG_PRICES if "PUBG" in t else (FF_PRICES if "Free" in t else (GS_PRICES if "Genshin" in t else ML_PRICES))
...
async def p_buy(c, s):
    k = c.data.replace("buy_", ""); it = {**PUBG_PRICES, **FF_PRICES, **GS_PRICES, **ML_PRICES}.get(k)
