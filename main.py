...
def main_menu(uid):
    ...
    builder.button(text="🎮 PUBG Mobile"), builder.button(text="🔥 Free Fire")
    builder.button(text="🪙 Roblox"), builder.button(text="❄️ Genshin Impact")
    builder.button(text="⚔️ Mobile Legends"), builder.button(text="💰 Hisob")
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    txt = (f"👋 Assalomu alaykum {message.from_user.full_name}, sizni bu botda korganmzdan xursandmiz!..\n\n"
           f"Arzon hamyonbob 🤑\nID orqali tushadi ✅\nIshonchli 😎")
    try:
        await message.answer_photo(photo=FSInputFile(MENYU_RASM), caption=txt, reply_markup=main_menu(message.from_user.id))
    except:
...
async def adm_users(message: types.Message):
    if message.from_user.id in ADMIN_IDS:
        conn = await get_db_conn()
        rows = await conn.fetch("SELECT user_id, full_name, balance FROM users LIMIT 50")
        await conn.close()
        if not rows:
            return await message.answer("Baza bo'sh.")
        text = "👥 <b>Foydalanuvchilar:</b>\n\n"
        for r in rows:
            text += f"🆔 <code>{r['user_id']}</code> | 👤 {r['full_name']} | 💰 {r['balance']:,} so'm\n"
...
async def adm_stats(message: types.Message):
    if message.from_user.id in ADMIN_IDS:
        conn = await get_db_conn()
        c = await conn.fetchval("SELECT COUNT(*) FROM users")
        b = await conn.fetchval("SELECT SUM(balance) FROM users")
        await conn.close()
        await message.answer(f"📊 <b>Statistika:</b>\n👥 Jami foydalanuvchilar: {c} ta\n💰 Jami balanslar: {b or 0:,} so'm")

@dp.message(Command("add"))
async def adm_add_cmd(message: types.Message):
    if message.from_user.id not in ADMIN_IDS: return
    try:
        a = message.text.split()
        uid, amount = int(a[1]), int(a[2])
        conn = await get_db_conn()
        await conn.execute("UPDATE users SET balance = balance + $1 WHERE user_id = $2", amount, uid)
        await conn.close()
        await message.answer("✅ Bajarildi!")
        await bot.send_message(uid, f"🎁 Balans: {amount} so'm qo'shildi/ayrildi.")
    except:
        await message.answer("Xato! /add [ID] [SUMMA]")
...
async def profile_cmd(message: types.Message, state: FSMContext):
    await state.clear()
    conn = await get_db_conn()
    bal = await conn.fetchval("SELECT balance FROM users WHERE user_id = $1", message.from_user.id)
    await conn.close()
    b = InlineKeyboardBuilder()
    b.button(text="➕ Hisobni to'ldirish", callback_data="topup_start")
...
async def topup_s(call: types.CallbackQuery):
    b = InlineKeyboardBuilder()
    b.button(text="VISA", callback_data="p_v"), b.button(text="HUMO", callback_data="p_h")
    await call.message.edit_text("To'lov turi:", reply_markup=b.as_markup())

@dp.callback_query(F.data.in_(["p_v", "p_h"]))
async def pay_i(call: types.CallbackQuery, state: FSMContext):
    t, k = ("VISA 💳", "4916 9903 4984 9908") if call.data=="p_v" else ("HUMO 💳", "9860 1606 2989 6350")
    await state.update_data(c_t=t)
    await call.message.edit_text(f"TOLDIRISH {t}\n\n<code>{k}</code>\nIsmoil Q***yev\n\nQancha tashlaysiz?", parse_mode="HTML")
...
async def get_am(message: types.Message, state: FSMContext):
    ...
    await state.update_data(am=int(message.text))
    w = await message.answer("⌛️ Kuting..."); await asyncio.sleep(5)
    b = InlineKeyboardBuilder(); b.button(text="✅ Tashladim", callback_data="i_s")
    await w.edit_text(f"Siz {message.text} so'm kiritdingiz.", reply_markup=b.as_markup())

@dp.callback_query(F.data == "i_s")
async def ask_p(call: types.CallbackQuery):
    await call.message.edit_text("⌛️ Ariza qabul qilinmoqda..."); await asyncio.sleep(6)
    await call.message.edit_text("📸 Chek rasmini yuboring:")
    await dp.fsm.get_context(bot, call.from_user.id, call.message.chat.id).set_state(DepositState.waiting_for_photo)

@dp.message(DepositState.waiting_for_photo, F.photo)
async def h_check(message: types.Message, state: FSMContext):
    d = await state.get_data()
    b = InlineKeyboardBuilder()
    b.button(text="✅ Tasdiqlash", callback_data=f"adm_p_app_{message.from_user.id}_{d['am']}")
    b.button(text="❌ Rad etish", callback_data=f"adm_p_rej_{message.from_user.id}")
    for a in ADMIN_IDS:
        try:
            await bot.send_photo(a, message.photo[-1].file_id, caption=f"📩 <b>To'lov!</b>\n👤 {message.from_user.full_name}\n🆔 {message.from_user.id}\n💰 {d['am']:,} so'm", parse_mode="HTML", reply_markup=b.as_markup())
        except: continue
    await message.answer("⌛️ Tekshirilmoqda kuting..."); await state.clear()

@dp.callback_query(F.data.startswith("adm_p_"))
async def adm_p_dec(call: types.CallbackQuery):
    p = call.data.split("_")
    act, uid = p[2], int(p[3])
    if act == "app":
        am = int(p[4])
        conn = await get_db_conn()
        await conn.execute("UPDATE users SET balance = balance + $1 WHERE user_id = $2", am, uid)
        await conn.close()
...
async def g_shop(m: types.Message, s: FSMContext):
    await s.clear()
    p_dict = PUBG_PRICES if "PUBG" in m.text else (FF_PRICES if "Fire" in m.text else (GENSHIN_PRICES if "Genshin" in m.text else ML_PRICES))
    b = InlineKeyboardBuilder(); [b.button(text=v['name'], callback_data=f"buy_{k}") for k, v in p_dict.items()]
    b.adjust(1); await m.answer(f"🛒 {m.text} paketini tanlang:", reply_markup=b.as_markup())

@dp.message(F.text == "🪙 Roblox")
async def rb_shop(m: types.Message, s: FSMContext):
    await s.clear()
    b = InlineKeyboardBuilder(); [b.button(text=v['name'], callback_data=f"buyrb_{k}") for k, v in ROBLOX_PRICES.items()]
    b.adjust(2); await m.answer("🪙 Roblox paketini tanlang:", reply_markup=b.as_markup())

@dp.callback_query(F.data.startswith("buyrb_"))
async def rb_c(call: types.CallbackQuery, state: FSMContext):
    it = ROBLOX_PRICES.get(call.data.replace("buyrb_", ""))
    await state.update_data(chosen=it['name'], pr=it['price'])
    b = InlineKeyboardBuilder(); b.button(text="✅ Sotib olaman", callback_data="rb_y"); b.button(text="⬅️ Orqaga", callback_data="back_m")
    await call.message.edit_text(f"❓ {it['name']} sotib olasizmi?", reply_markup=b.as_markup())

@dp.callback_query(F.data == "rb_y")
async def rb_a(call: types.CallbackQuery, state: FSMContext):
    await call.message.edit_text("📝 Nik va parol yozing!\nFormat: `nik: user parol: pass`\n⚠️ 2-tekshiruvni o'chiring!"); await state.set_state(ShopState.waiting_for_creds)

@dp.message(ShopState.waiting_for_creds)
async def rb_f(message: types.Message, state: FSMContext):
    d = await state.get_data(); b = InlineKeyboardBuilder(); b.button(text="✅ Tushdi", callback_data=f"ord_done_{message.from_user.id}_{d['pr']}_RB_none")
    for a in ADMIN_IDS:
        try: await bot.send_message(a, f"🪙 <b>ROBLOX!</b>\n👤 {message.from_user.full_name}\n🆔 {message.from_user.id}\n🔑 `{message.text}`\n💰 {d['pr']:,} so'm", parse_mode="HTML", reply_markup=b.as_markup())
        except: continue
    await message.answer("✅ Buyurtma qabul qilindi! 15 minutda tushadi."); await state.clear()

@dp.callback_query(F.data.startswith("buy_"))
async def p_buy(call: types.CallbackQuery, state: FSMContext):
    k = call.data.replace("buy_", ""); it = {**PUBG_PRICES, **FF_PRICES, **GENSHIN_PRICES, **ML_PRICES}.get(k)
    ...
    if bal and bal >= it['price']:
        await state.update_data(chosen=it['name'], pr=it['price'], key=k)
        await call.message.edit_text(f"✅ {it['name']}\n🔢 ID yuboring:"); await state.set_state(ShopState.waiting_for_id)
...
async def fin_b(message: types.Message, state: FSMContext):
    d = await state.get_data(); conn = await get_db_conn()
    promo = await conn.fetchval("SELECT promo_code FROM promos WHERE package_id = $1", d['key']); await conn.close()
    p_txt = f"\n🎁 Promo: <code>{promo}</code>" if promo else "\n🎁 Promo: Kiritilmagan"
    b = InlineKeyboardBuilder(); t_c = "PUBG" if "uc" in d['key'] else "GAME"
    b.button(text="✅ Tushdi", callback_data=f"ord_done_{message.from_user.id}_{d['pr']}_{t_c}_{d['key']}")
    for a in ADMIN_IDS:
        try: await bot.send_message(a, f"🛒 <b>Buyurtma!</b>\n👤 {message.from_user.full_name}\n🆔 {message.from_user.id}\n📦 {d['chosen']}\n🎮 ID: {d['p_id']}\n👤 Nick: {message.text}{p_txt}", parse_mode="HTML", reply_markup=b.as_markup())
        except: continue
    msg = "UC" if "uc" in d['key'] else ("Diamonds" if "ff" in d['key'] else "G.Crystals")
    await message.answer(f"✅ Buyurtma qabul qilindi! 15 minutda {msg} tushadi."); await state.clear()

@dp.callback_query(F.data.startswith("ord_done_"))
async def adm_c_o(call: types.CallbackQuery):
    p = call.data.split("_"); uid, pr, tp, pk = int(p[2]), int(p[3]), p[4], p[5]
    ...
    if bal and bal >= pr:
        ...
        await conn.close(); await bot.send_message(uid, "🎉 Buyurtmangiz tushdi ✅"); await call.message.edit_text(call.message.text + "\n\n✅ Bajarildi!", reply_markup=None)
    else: await conn.close(); await call.answer("Balans yetarli emas!", show_alert=True)

# --- PROMO ---
@dp.message(F.text == "🛠 Promo kiritish")
async def adm_pr(message: types.Message, state: FSMContext):
    await state.clear()
    if message.from_user.id in ADMIN_IDS:
        b = InlineKeyboardBuilder(); [b.button(text=v['name'], callback_data=f"sp_{k}") for k, v in PUBG_PRICES.items()]
        b.adjust(1); await message.answer("Promo paketini tanlang:", reply_markup=b.as_markup())

@dp.callback_query(F.data.startswith("sp_"))
async def adm_s_p(call: types.CallbackQuery, state: FSMContext):
    await state.update_data(pk=call.data.replace("sp_", "")); await call.message.edit_text("Promo kodni yozing:"); await state.set_state(AdminPromoState.waiting_for_promo_code)

@dp.message(AdminPromoState.waiting_for_promo_code)
async def adm_sv_p(message: types.Message, state: FSMContext):
    d = await state.get_data(); conn = await get_db_conn()
    await conn.execute('INSERT INTO promos (package_id, promo_code) VALUES ($1, $2) ON CONFLICT (package_id) DO UPDATE SET promo_code = $2', d['pk'], message.text)
    await conn.close(); await message.answer("✅ Promo saqlandi!"); await state.clear()

@dp.message(F.text == "⬅️ Orqaga")
@dp.callback_query(F.data == "back_m")
async def b_m(event, state: FSMContext):
    await state.clear()
