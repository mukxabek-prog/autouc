...
# SUPABASE URI (Parol bilan birga tayyor holatda)
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    start_text = (
        ...
        f"Undan tashqar :\n"
        f"Free Firee uchu ham diamondlar💎\n"
        f"Olishingiz mumkin 🫵"
    )
    try:
        await message.answer_photo(photo=FSInputFile(MENYU_RASM), caption=start_text, reply_markup=main_menu(message.from_user.id))
    except:
